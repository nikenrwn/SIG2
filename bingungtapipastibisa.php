          ...
          function addMarker(lat, lng, info) {
            ...
            if(isset($_POST['namafasilitas'])) {
              ...
              $query = mysql_query("SELECT nama_lenm, jenis_lenm, id_lenm,lat_lenm, long_lenm,no_fas 
                ...
                $sql = "SELECT DISTINCT(id_reklame), L.jumlah_pengguna, R.SIPR, R.status, R.alamat, R.id_status,
                        R.id_jalan, R.lat_reklame, R.long_reklame,
                    (6371 * acos(cos(radians(".$lat.")) 
                    * cos(radians(lat_reklame)) * cos(radians(long_reklame) 
                    - radians(".$lon.")) + sin(radians(".$lat.")) 
                    * sin(radians(lat_reklame)))) 
                    AS jarak 
                    FROM tb_reklame3 R, lhs_rungkut L, tb_jenis_jalan J
                    HAVING jarak <= 0.4 AND R.id_status=2 AND R.id_jalan = J.id_status and J.id_status = ".$kepadatan."
                    ORDER BY jarak";
                                  ...
                                  $json2 .= '"niken2":[ ';
                                    while($y = mysql_fetch_array($databaru)){
                                      ...
                                      $json2 .= '"id_reklame":"'.$y['id_reklame'].'",
                                           ...
                                           "alamat":"'.htmlspecialchars_decode($y['alamat']).'",
  ...
  <body>
    ...
    <nav class="navbar navbar-inverse navbar-fixed-top">
      ...
      <li class="dropdown">
              ...
              <ul class="dropdown-menu">
                <li><a href="#">Analisis Kriteria</a></li>
        ...
        <?php
            if(isset($_POST['namafasilitas'])) {
              ...
              $query = mysql_query("SELECT nama_lenm, jenis_lenm, id_lenm,lat_lenm, long_lenm,no_fas 
                ...
                $sql = "SELECT DISTINCT(id_reklame), SIPR, status,alamat, id_status, lat_reklame, long_reklame,
                    (6371 * acos(cos(radians(".$lat.")) 
                    * cos(radians(lat_reklame)) * cos(radians(long_reklame) 
                    - radians(".$lon.")) + sin(radians(".$lat.")) 
                    * sin(radians(lat_reklame)))) 
...
//                                    echo $json;
                                  ...
                                  if(mysql_fetch_array($databaru)){
                                   ...
                                   echo "<td>".htmlspecialchars_decode($x['alamat'])."</td>";
